...
var photoAndComment = require('./schema/photo.js');
var Photo = photoAndComment.Photo;
var Comments = photoAndComment.Comment;
...
Promise.all(removePromises).then(function () {
    ...
    var userPromises = userModels.map(function (user) {
        ...
        var newPasswordEntry = cs142password.makePasswordEntry('weak');
        return User.create({
            ...
            password_digest: newPasswordEntry.hash,
            salt: newPasswordEntry.salt
    ...
    var allPromises = Promise.all(userPromises).then(function () {
        ...
        var photoPromises = photoModels.map(function (photo) {
            var favoriteIDs = [];
            for(var k = 0; k < photo.favoritedBy.length; k++)
            {
              favoriteIDs.push(photo.favoritedBy[k].objectID);
            }
            ...
            return Photo.create({
                ...
                if (err) {
                    ...
                    photoObj.save();
                    allPhotoModels.push(photoObj);
                    photo.objectID = photoObj._id;
                    HistoryInfo.create({
                        user_id: photoObj.user_id,
                        comment: "",
                        type: "photo",
                        photoInvolved: photoObj.id,
                        date_time: photoObj.date_time,
                      },
                      function()
                      {
                        console.log("Added photo ", photoObj.id," to history");
                      });

                    if (photo.comments) {
                        photo.comments.forEach(function (comment) {
                            var objGiven = new ObjectId();
                            photoObj.comments.push({
                                comment: comment.comment,
                                date_time: comment.date_time,
                                user_id: comment.user.objectID,
                                belongsToPhoto: photoObj._id,
                                second_id: objGiven
                            });
                            ...
                            Comments.create({
                              ...
                              belongsToPhoto: photoObj._id,
                              second_id: objGiven
                            }, function(err, actualComment)
                            {
                              commentModel.push(actualComment);
                              console.log("Added comment ", actualComment.second_id, " to the comments");
                            });
                            ...
                            HistoryInfo.create({
                              ...
                              comment: objGiven,
                              type: "comment",
                              photoInvolved: photoObj._id,
                              date_time: comment.date_time,
                            },
                            function()
                            {
                              console.log("Added comment ", objGiven, " to the history table");
                            });
